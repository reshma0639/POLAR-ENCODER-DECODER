...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{./figs/pmf_dice.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{./figs/log.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{./figs/triangle_cdf.pdf}
...
\begin{lstlisting}
probability/codes/chapter_2/2.4.5_pdf.py
\end{lstlisting}
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{./figs/triangle_pdf.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{./figs/bpsk_pe_snr.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{./figs/bpsk_map_density.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{./figs/chisq_cdf.pdf}
\caption{CDF of $V$}
\label{fig:chisq_cdf}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{./figs/pdf.pdf}
\caption{PDF of $V$}
\label{fig:chisq_pdf}
\end{figure}
%
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{./figs/root_cdf.pdf}
\caption{CDF of $A$}
\label{fig:rayleigh_cdf}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{./figs/chisq_pdf.pdf}
\caption{PDF of $A$}
...
\solution \\
In order to find the decession rule for detecting the signals Let us first discuss with the FSK modulation and demodulation. \\

	\begin{tikzpicture}[block/.style={rectangle, draw, fill=white, text width=5em, text centered, rounded corners}, line/.style={draw, -latex}]
\centering
		\node [block] (message) {Message};
		\node [block, right of=message, xshift=3cm] (modulator) {FSK Modulator};
		\node [block, right of=modulator, xshift=3cm] (transmitter) {Transmitter};
		\node [block, below of=transmitter, yshift=-2cm] (channel) {Channel};
		\node [block, below of=channel, yshift=-2cm] (receiver) {Receiver};
		\node [block, left of=receiver, xshift=-3cm] (demodulator) {FSK Demodulator};
		\node [block, left of=demodulator, xshift=-3cm] (original) {Original Message};
		
		\path [line] (message) -- (modulator);
		\path [line] (modulator) -- (transmitter);
		\path [line] (transmitter) -- (channel);
		\path [line] (channel) -- (receiver);
		\path [line] (receiver) -- (demodulator);
		\path [line] (demodulator) -- (original);
		
		\end{tikzpicture}	
\begin{enumerate}	
\item \textbf{FSK Modulation} \\
The frequency of a sinusoidal carrier shifted according to the binary symbol.The frequency of a sinusoidal carrier is shifted between two discerte values.\vspace{3mm} \\
This mean that we have two different frequency signals according to binary symbols.\vspace{3mm} \\
Consider the symbols to be transmited is 0 and 1 \\
let the two carrier signals is \\
\begin{enumerate}
    \item For symbol $s_0$  the carrier is $A_c$cos(2$\pi$$f_1$t) 
    \item For symbol $s_1$  the carrier is $A_c$cos(2$\pi$$f_2$t)
    \item $f_1$ is the frequency of FSK at symbol $s_0$
    \item $f_2$ is the frequency of FSK at symbol $s_1$
\end{enumerate}
\vspace{3mm}
The block diagram of FSK is given as :
\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{/home/mannava/latex/fsk_mod.png}
    \label{fsk modulation block diagram}
\end{figure}

  
Let the output of FSK be the s(t) \\
Let P,$E_b$ be the power and energy of the signal \\
Consider the carrier signal  \\

\begin{align}
	C_1(t) &= A_c\cos(2\pi f_1t) \\
	C_2(t) &= A_c\cos(2\pi f_2t) \\
	P &= \frac{A_c^2}{2} \\
	A_c &= \sqrt{2P} \\
	C_1(t) &= \sqrt{E_b}\phi_1(t) \\
	\phi_1(t) &= \sqrt{\frac{2}{T_b}}\cos(2\pi f_1t) \\
\end{align}
Similarly \\
\begin{align}
	C_2(t) &= \sqrt{E_b}\phi_2(t) \\
	\phi_2(t) &= \sqrt{\frac{2}{T_b}}\cos(2\pi f_2t) \\
\end{align}
\textbf{For symbol $s_0$} \\
let \\
\begin{align}
	S(t) &= S_1(t)+S_2(t) \\
	\vec{s}_0 &= 
	...
	\begin{pmatrix}
		...
		\vec{s_0^T}
			\begin{pmatrix}
				...
				\end{pmatrix}
\end{align} 
Finally for symbol $s_0$\\
\begin{align}
	S(t)&=A_c\cos(2\pi f_1t) \\
\end{align}
Similarly \\
\textbf{For symbol $s_1$} \\
\begin{align}
	S(t)&=A_c\cos(2\pi f_2t) \\
\end{align}
\textbf{FSK demodulation}\\
Block diagram of FSK Demodulation is given as:\\
\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{/home/mannava/latex/fsk_demod.png}
    \label{fsk modulation block diagram}
\end{figure}
For the above figure The input is S(t) \\
\textbf{For symbol $s_0$} \\
\begin{align}
S(t)&=A_c\cos(2\pi f_1 t) 
\end{align}
let the output of multiplier is $p_1(t)$
The output of multiplier is : \\
\begin{align}
	p_1(t)=A_c^2cos^2(2\pi f_1 t)
\end{align}
The output of lowpass filter is : $\frac{A_c^2}{2}$ \\
On the other side the output of second multiplier is : \\
\begin{align}
	p_2(t)=A_c^2 \cos(2\pi f_1 t)\cos(2\pi f_2 t)
\end{align}
The output of second low pass filter is 0 \\
The both outputs of low pass filter is feeded to the comparator and hence it detects the message signal sent.\\
Finally the symbol $s_0$ was detected.\\
Similarly \\
The symbol $s_1$ was also be detected.\\
\vspace{5mm}
\textbf{Signal space diagram for fsk} \\
For symbol $s_0$ 
\begin{align}
S(t)&=A_c\cos(2\pi f_1 t) \\
...
For symbol $s_1$
\begin{align}
S(t)&=A_c\cos(2\pi f_2 t) \\
S(t)&=\sqrt{E_b}\phi_2(t)
\end{align}
$\phi_1$(t) and $\phi_2$(t) are orthogonal functions \\
The constellation diagram for FSK is given as : \\
\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth/2]{/home/mannava/latex/cons.jpg}
    \label{fsk modulation block diagram}
\end{figure}
\textbf{Detection Theory with 2 Dimensional Signals} \\
The transmited symbols be $s_0$,$s_1$.Recall $\vec{s_i}$ vectors are length 2,we write elements as: \\
\begin{align}
	\vec{s_i}=\myvec{s_0\\s_1}
\end{align}
At the receiver side the symbol vector plus noise,after the matched filter and downsampler.\\
\begin{align}
	H_0 : \vec{X}=s_0+\vec{W}\\
	H_1 : \vec{X}=s_1+\vec{W}
\end{align}
Assume : $\vec{W}$ is multivariate Gaussian,each of K components $W_k$ are independent with zero mean and variance $\sigma^2_w$ = $\frac{N_0}{2}$ \\
Assume : Symbols are equally likely.\vspace{5mm} \\
\textbf{Optimal Detection with Multiple Hypothesis}\\
The measurement $\vec{X}$ has a different joint probability density under each hypothesis: \\
\begin{align}
	H_0 : f_{x|H_0}(\vec{x}|H_0)\Pr[H_0] \\
	H_1 : f_{x|H_1}(\vec{x}|H_1)\Pr[H_1] 
\end{align}
The probability of error is minimized by finding the i which joint probability of x and $H_i$ for i $\epsilon$ {0,1,2 ..... M-1} is highest \\
That is ,we decide symbol i was sent if.\\
\begin{align}
f_{x/H_i}(\vec{x}|H_i)\Pr[H_i] > f_{x/H_j}(\vec{x}/H_j)\Pr[H_j] 
\end{align}
\begin{align}
Decision &=\arg \max f_{x|H_i}(\vec{x}|H_i)
\end{align}
\begin{align}
f_{x|H_i}(x|H_i) &= \frac{1}{\sqrt[K-1]{2\pi\sigma^2_w}}\exp -\sum_{i = 0}^{K-1}\frac{(\vec{x_k}-\vec{s_i})^2}{2\sigma^2_w}  \\
K&=2
\end{align}
\begin{align}
f_{x|H_i}(x|H_i) &= \frac{1}{\sqrt[K-1]{2\pi\sigma^2_w}}\exp -\frac{\left\lVert \vec{x}-\vec{s_i}\right\rVert ^2}{2\sigma^2_w}  
\end{align}
\begin{align}
	f_{x|H_i}(x|H_i) &= \frac{1}{2\pi\sigma^2_w}\exp -\frac{\left\lVert \vec{x}-\vec{s_i}\right\rVert ^2}{2\sigma^2_w}  
	\end{align}
\begin{align}
\hat{i} &= \arg\max\biggl\{  \log \frac{1}{2\pi\sigma^2_w}-\frac{\left\lVert \vec{x}-\vec{s_i}\right\rVert ^2}{2\sigma^2_w} \biggr\}\\
\hat{i} &= \arg\max-\frac{\left\lVert \vec{x}-\vec{s_i}\right\rVert ^2}{2\sigma^2_w} \\
\hat{i} &= \arg\min\left\lVert \vec{x}-\vec{s_i}\right\rVert ^2 \\
\hat{i}  &= \arg\min\left\lVert \vec{x}-\vec{s_i}\right\rVert
\end{align}
Find the locus of points $\vec{x}$ which satisfy the equality of distances between the two symbol points: \\
\begin{align}
	\left\lVert \vec{x}-\vec{s_0}\right\rVert ^2&=\left\lVert \vec{x}-\vec{s_1}\right\rVert ^2 \\
	(\vec{x}-\vec{s_0})^T(\vec{x}-\vec{s_0})&=(\vec{x}-\vec{s_1})^T(\vec{x}-\vec{s_1}) \\
	\vec{x}^T\vec{x} -\vec{x}^T\vec{s_0}-\vec{s_0}^T\vec{x}+\vec{s_0}^T\vec{s_0} &= \vec{x}^T\vec{x} -\vec{x}^T\vec{s_1}-\vec{s_1}^T\vec{x}+\vec{s_1}^T\vec{s_1} \\
	(\vec{s_1}-\vec{s_0})^T\vec{x}&=\frac{\left\lVert \vec{s_1}\right\rVert ^2-\left\lVert \vec{s_0}\right\rVert ^2}{2}
\end{align}
According to the given question \\
\begin{align}
	\left\lVert \vec{s_1}\right\rVert ^2&=\left\lVert \vec{s_0}\right\rVert ^2
\end{align}
Hence the decesion rule for bivariate FSK is : \\
\begin{align}
	(\vec{s_1}-\vec{s_0})^T\vec{x}&=0
\end{align}
\begin{align}
	\vec{s_1}&=\myvec{0 \\ 1} \\
	\vec{s_0}&=\myvec{1 \\ 0} \\
\end{align}
By substituting $\vec{s_0}$ and $\vec{s_1}$ in above expression
\begin{align}
	(\vec{s_1}-\vec{s_0})^T\vec{x}&=0\\
	\begin{pmatrix}
-1\\
...
\solution The red dots in \figref{fig:biv_pe_snr} are the $P_e$ versus SNR plot. It is generated using the below code,
\begin{lstlisting}
